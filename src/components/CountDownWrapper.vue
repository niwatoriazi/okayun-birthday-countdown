...
setInterval(function () {
  const distance = dayjs("2023-02-22 00:00:00").diff(dayjs());
...
<template>
  ...
  >
    ...
    >
      <span class="lg:text-7xl text-3xl">誕生日まであと</span><br />{{ getDay
      }}<span class="lg:text-7xl text-3xl">日</span><br class="lg:hidden" />{{
        getHour
      }}<span class="lg:text-7xl text-3xl">時間</span><br />{{ getMinute
      }}<span class="lg:text-7xl text-3xl">分</span><br class="lg:hidden" />{{
        getSecond
      }}<span class="lg:text-7xl text-3xl">秒</span>
    </h1>
  </div>
</template>
<style lang="scss">
